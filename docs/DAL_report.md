...
## Что такое подготовленные выражения (prepared statements) и как
они предотвращают SQL-инъекции? Покажите на примере вашего кода.

**Подготовленные выражения (Prepared Statements)** — это механизм БД, при котором структура SQL-запроса и данные передаются серверу раздельно. Сначала отправляется шаблон с плейсхолдерами, затем — массив значений.

**Как это защищает от SQL-инъекций:**
База данных воспринимает переданные параметры строго как данные, а не как исполняемый код. Вредоносный ввод (например, '; DROP TABLE users;) автоматически экранируется и записывается как обычный текст, не ломая логику запроса.

Пример из моего кода (AppointmentRepository.php):

```php
// 1. Отправляем только структуру запроса
$stmt = $this->pdo->prepare("SELECT * FROM appointments WHERE status = ?");

// 2. Данные передаются отдельно и не интерпретируются как SQL
$stmt->execute([$_GET['status']]); 
```
Такой подход полностью исключает возможность внедрения
вредоносных команд через пользовательский ввод.